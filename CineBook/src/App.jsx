/*
  ╔══════════════════════════════════════════════════════════════════╗
  ║  CINEBOOK — Netflix-style Movie Booking App                      ║
  ║  Drop this file as src/App.jsx in a Vite + React project         ║
  ║  Run: npm create vite@latest cinebook -- --template react        ║
  ║       cd cinebook && npm install && npm run dev                   ║
  ╚══════════════════════════════════════════════════════════════════╝
*/
...
@keyframes glow     { 0%,100%{box-shadow:0 0 20px rgba(229,9,20,.3)} 50%{box-shadow:0 0 50px rgba(229,9,20,.6)} }
...
.btn-red:hover:not(:disabled) { background: #f40612; transform: scale(1.03); }
.btn-ghost { background: rgba(120,120,120,.45); color: #fff; }
.btn-ghost:hover { background: rgba(120,120,120,.7); }
...
.btn-danger { background: rgba(229,9,20,.12); color: #f87171; border: 1px solid rgba(229,9,20,.3); }
...
.inp:focus { border-color: var(--red); box-shadow: 0 0 0 3px rgba(229,9,20,.12); }
...
.pill {
  ...
  font-size: 13px; font-weight: 600; cursor: pointer; transition: all .2s; border: 1.5px solid transparent; white-space: nowrap;
}
.pill-ghost  { background: rgba(255,255,255,.05); border-color: rgba(255,255,255,.1); color: var(--text3); }
.pill-ghost:hover { background: rgba(255,255,255,.1); color: var(--text); }
...
.tbl td { padding: 13px 14px; border-bottom: 1px solid #111; color: var(--text2); font-size: 14px; vertical-align: middle; }
...
function RouterProvider({ children }) {
  ...
  return (
    <RouterCtx.Provider value={{
      page: cur.page, params: cur.params,
...
function ToastProvider({ children }) {
  ...
  const typeConfig = {
    success: { bg: "#14532d", border: "#166534", icon: <ICONS.Check s={15}/> },
    error:   { bg: "#7f1d1d", border: "#991b1b", icon: <ICONS.Alert s={15}/> },
    info:    { bg: "#78350f", border: "#92400e", icon: <ICONS.Info  s={15}/> },
    warning: { bg: "#713f12", border: "#854d0e", icon: <ICONS.Alert s={15}/> },
  };
        ...
        {toasts.map(t => {
          ...
          return (
            <div key={t.id} className="slide-up" style={{
              ...
              display: "flex", alignItems: "center", gap: 10, boxShadow: "0 8px 40px rgba(0,0,0,.8)",
            }}>
              {cfg.icon}
...
const Spinner = ({ size = 36 }) => (
  <div style={{ display: "flex", justifyContent: "center", alignItems: "center", padding: 56 }}>
    <div style={{ width: size, height: size, border: "3px solid #1e1e1e", borderTop: "3px solid #e50914", borderRadius: "50%", animation: "spin .8s linear infinite" }}/>
  </div>
);
...
const Empty = ({ icon = "🎬", title, sub, action }) => (
  <div style={{ textAlign: "center", padding: "80px 24px" }}>
    ...
    <p style={{ fontSize: 20, fontWeight: 700, color: "#444", marginBottom: 8 }}>{title}</p>
    {sub && <p style={{ fontSize: 14, color: "#333" }}>{sub}</p>}
...
const StatusBadge = ({ status }) => {
  const cfg = {
    CONFIRMED: { bg: "rgba(74,222,128,.12)", c: "#4ade80", bc: "rgba(74,222,128,.3)" },
    PENDING:   { bg: "rgba(251,191,36,.12)", c: "#fbbf24", bc: "rgba(251,191,36,.3)" },
    CANCELLED: { bg: "rgba(248,113,113,.12)", c: "#f87171", bc: "rgba(248,113,113,.3)" },
    FAILED:    { bg: "rgba(107,114,128,.12)", c: "#6b7280", bc: "rgba(107,114,128,.3)" },
    UPCOMING:  { bg: "rgba(96,165,250,.12)", c: "#60a5fa", bc: "rgba(96,165,250,.3)" },
    RUNNING:   { bg: "rgba(74,222,128,.12)", c: "#4ade80", bc: "rgba(74,222,128,.3)" },
    COMPLETED: { bg: "rgba(107,114,128,.12)", c: "#6b7280", bc: "rgba(107,114,128,.3)" },
  };
  const s = cfg[status] || cfg.FAILED;
  return (
    <span className="badge" style={{ background: s.bg, color: s.c, border: `1px solid ${s.bc}` }}>
      {status}
    </span>
  );
};
...
const SectionTitle = ({ children }) => (
  <h2 style={{ fontSize: 20, fontWeight: 800, color: "#ddd", display: "flex", alignItems: "center", gap: 10 }}>
    {children}
  </h2>
);
  ...
  <div className="modal-bg" onClick={e => e.target === e.currentTarget && onClose()}>
    <div className="scale-in" style={{
      ...
      <button onClick={onClose} style={{
        position: "absolute", top: 14, right: 14, background: "#1e1e1e",
        border: "none", borderRadius: "50%", width: 33, height: 33,
        cursor: "pointer", color: "#777", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2,
      }}>
        <ICONS.X s={16}/>
      </button>
      {children}
...
function MovieCard({ movie, onClick, width = 178 }) {
  ...
  return (
    <div className="mcard" style={{ width, flexShrink: 0 }} onClick={onClick}
      ...
      {/* Poster */}
      <div style={{ paddingTop: "150%", background: "#141414", position: "relative" }}>
        {movie.posterUrl
          ? <img src={movie.posterUrl} alt={movie.title}
              style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }}
              onError={e => { e.target.style.display = "none"; }}/>
          : <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 48, color: "#222" }}>🎬</div>
        }
        ...
        <div style={{
          ...
          <div style={{ position: "absolute", bottom: 0, padding: "14px 12px", width: "100%" }}>
            ...
            <div style={{ display: "flex", gap: 7, flexWrap: "wrap", alignItems: "center" }}>
              {movie.rating && (
                <span style={{ color: "#f5c518", fontSize: 12, fontWeight: 800, display: "flex", alignItems: "center", gap: 3 }}>
                  <ICONS.Star s={11} color="#f5c518"/>{movie.rating.toFixed(1)}
                </span>
              )}
        ...
        {movie.certificate && (
          <div style={{
            ...
            background: "#e50914", borderRadius: 4,
    ...
    <div style={{ marginBottom: 44 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 52px", marginBottom: 8 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {[-1, 1].map(dir => (
            <button key={dir} className="btn btn-outline btn-icon" onClick={() => scroll(dir)}
              style={{ width: 32, height: 32, background: "rgba(255,255,255,.05)" }}>
  ...
  return (
    <div style={{ position: "relative", height: "90vh", minHeight: 560, overflow: "hidden" }}>
      {movie.posterUrl
        ...
        : <div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg,#1a0000,#080808)" }}/>
      }
      {/* Cinematic overlays */}
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right, rgba(8,8,8,.98) 0%, rgba(8,8,8,.7) 42%, rgba(8,8,8,.08) 100%)" }}/>
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "52%", background: "linear-gradient(to top, #080808, transparent)" }}/>
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "25%", background: "linear-gradient(to bottom, rgba(8,8,8,.6), transparent)" }}/>
      ...
      <div className="fade-up" style={{ position: "absolute", bottom: 0, left: 0, padding: "0 56px 76px", maxWidth: 660 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 18, flexWrap: "wrap" }}>
          ...
          {movie.genre       && <span style={{ border: "1px solid rgba(255,255,255,.25)", color: "#ccc", fontSize: 12, padding: "3px 10px", borderRadius: 4 }}>{movie.genre}</span>}
          {movie.language    && <span style={{ border: "1px solid rgba(255,255,255,.15)", color: "#999", fontSize: 12, padding: "3px 10px", borderRadius: 4 }}>{movie.language}</span>}
        </div>

        <h1 className="display" style={{ fontSize: "clamp(44px, 7.5vw, 90px)", lineHeight: .9, marginBottom: 18, textShadow: "0 4px 32px rgba(0,0,0,.6)" }}>
          {movie.title}
        </h1>
        ...
        <p style={{ color: "#999", fontSize: 15, lineHeight: 1.75, marginBottom: 30, display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
          {movie.description}
        </p>
    ...
    <Modal onClose={onClose}>
      <div style={{ padding: "48px 44px 42px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <p style={{ color: "#555", fontSize: 14 }}>{mode === "login" ? "Sign in to your account" : "Create your account free"}</p>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {mode === "register" && (
            <Field label="Phone">
              <input className="inp" placeholder="10-digit mobile number" value={form.phone} onChange={set("phone")}/>
            </Field>
          )}
          <button className="btn btn-red btn-md" onClick={submit} disabled={loading}
...
function Navbar({ onAuthOpen }) {
  ...
  const { page, navigate, replace } = useRouter();
  const toast = useToast();
  ...
  return (
    <nav style={{
      ...
      background: scrolled ? "rgba(8,8,8,.97)" : "linear-gradient(to bottom, rgba(0,0,0,.7), transparent)",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
        <div onClick={() => go("home")} className="display"
          style={{ fontSize: 28, letterSpacing: 4, color: "#e50914", cursor: "pointer", userSelect: "none" }}>
          CINEBOOK
        </div>
        {user && (
          ...
          <div style={{ position: "relative" }}>
            ...
            {menu && (
              <div className="scale-in" style={{
                ...
                background: "#161616", border: "1px solid #222", borderRadius: 13,
                ...
                {[
                  ...
                  <button key={item.p} onClick={() => go(item.p)} style={{
                    ...
                    padding: "10px 14px", cursor: "pointer", borderRadius: 7, fontSize: 14, transition: "background .15s",
                  }}
                ...
                <button onClick={() => { logout(); setMenu(false); replace("home"); toast("Signed out", "success"); }} style={{
                  ...
                  padding: "10px 14px", cursor: "pointer", borderRadius: 7, fontSize: 14,
                }}
...
function PreviewModal({ movie, onClose, onBookNow }) {
  ...
  useEffect(() => {
    api.get(`/api/shows/movie/${movie.id}`)
      .then(setShows).catch(() => setShows([])).finally(() => setLoading(false));
  }, [movie.id]);

  const goDetail = () => { onClose(); navigate("movie", { movie }); };

  const minPrice = s => Math.min(...[s.silverPrice, s.goldPrice, s.platinumPrice, s.reclinerPrice].filter(Boolean));

  return (
    <Modal onClose={onClose} maxW={860}>
      {/* Hero image */}
      <div style={{ position: "relative", height: 360, overflow: "hidden", borderRadius: "18px 18px 0 0" }}>
        {movie.posterUrl
          ? <img src={movie.posterUrl} alt={movie.title} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 20%" }}/>
          : <div style={{ width: "100%", height: "100%", background: "#111" }}/>
        }
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, #111 0%, rgba(17,17,17,.15) 55%, transparent 100%)" }}/>
      ...
      <div style={{ padding: "22px 28px 28px" }}>
        <div style={{ display: "flex", gap: 28 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", marginBottom: 14 }}>
              ...
              {movie.certificate && <span style={{ border: "1px solid #333", color: "#ccc", fontSize: 11, padding: "2px 7px", borderRadius: 3 }}>{movie.certificate}</span>}
          ...
          <div style={{ width: 185, flexShrink: 0, fontSize: 13, lineHeight: 2.4 }}>
            {[["Director", movie.director], ["Cast", movie.cast], ["Language", movie.language]].map(([l, v]) => (
              <div key={l}><span style={{ color: "#444" }}>{l}: </span><span style={{ color: "#ccc" }}>{v || "—"}</span></div>
            ))}
          </div>
        </div>
        ...
        {!loading && shows.length > 0 && (
          <div style={{ marginTop: 24 }}>
            <p style={{ color: "#444", fontSize: 11, fontWeight: 700, letterSpacing: 1, marginBottom: 12 }}>TODAY'S SHOWS</p>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {shows.slice(0, 5).map(s => (
                <div key={s.id} onClick={goDetail} style={{
                  background: "#1a1a1a", border: "1.5px solid #222", borderRadius: 10,
                  padding: "10px 16px", cursor: "pointer", transition: "all .2s",
                }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = "#e50914"; e.currentTarget.style.background = "#1c0f0f"; }}
                  ...
                  <div style={{ fontSize: 11, color: "#666", marginTop: 2 }}>{s.theatreName}</div>
                  <div style={{ fontSize: 12, color: "#e50914", fontWeight: 800, marginTop: 3 }}>from ₹{minPrice(s)}</div>
                </div>
              ))}
            </div>
          </div>
...
function SeatMap({ showId, onSeatsChange }) {
  ...
  useEffect(() => {
    ...
    api.get(`/api/shows/${showId}/seats`)
      .then(setSeats).catch(() => setSeats([])).finally(() => setLoading(false));
  }, [showId]);
  ...
  const seatBg = seat => {
    ...
    if (seat.status === "BOOKED") return "#181818";
  ...
  const grouped = useMemo(() =>
    seats.reduce((acc, s) => { (acc[s.rowNumber] ||= []).push(s); return acc; }, {}),
    [seats]
  );
    ...
    <div>
      ...
      <div style={{ textAlign: "center", marginBottom: 38 }}>
        <div style={{
          ...
          background: "linear-gradient(90deg, transparent, rgba(229,9,20,.45), rgba(229,9,20,.85), rgba(229,9,20,.45), transparent)",
          borderRadius: 4, boxShadow: "0 0 40px rgba(229,9,20,.35), 0 0 100px rgba(229,9,20,.12)", marginBottom: 10,
        }}/>
        <div style={{ color: "#333", fontSize: 11, letterSpacing: 7, textTransform: "uppercase" }}>All Eyes Here</div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 7, alignItems: "center" }}>
        {Object.entries(grouped).sort().map(([row, rowSeats]) => (
          <div key={row} style={{ display: "flex", gap: 5, alignItems: "center" }}>
            <span style={{ color: "#2e2e2e", fontSize: 11, width: 22, textAlign: "right", marginRight: 8 }}>{row}</span>
              ...
              {rowSeats.sort((a, b) => a.seatNumber.localeCompare(b.seatNumber)).map(seat => (
                <button key={seat.showSeatId} className="seat"
                  ...
                  style={{
                    ...
                    opacity: seat.status === "BOOKED" ? .28 : 1,
      ...
      <div style={{ display: "flex", gap: 18, justifyContent: "center", marginTop: 28, flexWrap: "wrap" }}>
        {[
          ...
          ["#14532d", "Recliner"],
          ["#e50914", "Selected"],
          ["#181818", "Booked", "1px solid #2a2a2a"],
        ].map(([col, lbl, bdr]) => (
...
function HomePage({ onAuthRequired }) {
  ...
  const openMovie  = m => navigate("movie", { movie: m });
    ...
    <div>
      ...
      {!loading && hero && (
        <HeroBanner movie={hero}
          onBook={() => openMovie(hero)}
          onInfo={() => openPreview(hero)}/>
      )}
      ...
      <div style={{ padding: "24px 52px 12px", display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ position: "relative", flex: "0 0 270px" }}>
          <div style={{ position: "absolute", left: 13, top: "50%", transform: "translateY(-50%)", color: "#555", pointerEvents: "none" }}>
            <ICONS.Search s={16}/>
          </div>
      ...
      {search || genre !== "All" ? (
        <div style={{ padding: "12px 52px 80px" }}>
          <p style={{ color: "#444", fontSize: 13, marginBottom: 22 }}>{filtered.length} result{filtered.length !== 1 ? "s" : ""}</p>
          {filtered.length === 0
            ? <Empty icon="🔍" title="Nothing matched" sub="Try a different title or genre"/>
            : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(165px, 1fr))", gap: 12 }}>
                {filtered.map(m => <MovieCard key={m.id} movie={m} width="100%" onClick={() => openPreview(m)}/>)}
              </div>
          }
        </div>
      ) : (
        <div style={{ paddingBottom: 80 }}>
          {loading ? (
            <MovieRow title="🔥 Trending Now" loading/>
          ) : (
            <>
              <MovieRow title="🔥 Trending Now"  movies={movies.slice(0, 18)} onMovieClick={openPreview}/>
              <MovieRow title="⭐ Top Rated"      movies={topRated}            onMovieClick={openPreview}/>
              <MovieRow title="🆕 New Releases"  movies={newRelease}          onMovieClick={openPreview}/>
              {Object.entries(byGenre).map(([g, gm]) => (
                <MovieRow key={g} title={g} movies={gm} onMovieClick={openPreview}/>
              ))}
            </>
          )}
        </div>
...
function MoviePage() {
  ...
  useEffect(() => {
    ...
    api.get(`/api/shows/movie/${movie.id}?date=${date}`)
      .then(setShows).catch(() => setShows([])).finally(() => setLoadShows(false));
  }, [movie?.id, date]);
  ...
  const dates = [...Array(7)].map((_, i) => {
    ...
    return { val: d.toISOString().split("T")[0], label: i === 0 ? "Today" : i === 1 ? "Tomorrow" : d.toLocaleDateString("en-IN", { weekday: "short", day: "numeric", month: "short" }) };
  });
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ position: "relative", height: "60vh", overflow: "hidden" }}>
        {movie.posterUrl && (
          <img src={movie.posterUrl} alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 18%", filter: "brightness(.3)" }}/>
        )}
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, rgba(8,8,8,.12) 0%, #080808 100%)" }}/>
        ...
        <div style={{ position: "absolute", bottom: 0, left: 0, padding: "0 56px 40px", display: "flex", gap: 34, alignItems: "flex-end" }}>
          {movie.posterUrl && (
            <img src={movie.posterUrl} alt={movie.title} style={{
              width: 158, height: 237, objectFit: "cover", borderRadius: 12,
              boxShadow: "0 24px 64px rgba(0,0,0,.95)", flexShrink: 0,
          ...
          <div style={{ paddingBottom: 6 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
              ...
              {movie.genre       && <span style={{ border: "1px solid #333", color: "#bbb", fontSize: 12, padding: "3px 10px", borderRadius: 4 }}>{movie.genre}</span>}
              {movie.language    && <span style={{ border: "1px solid #333", color: "#bbb", fontSize: 12, padding: "3px 10px", borderRadius: 4 }}>{movie.language}</span>}
            </div>
            <h1 className="display" style={{ fontSize: "clamp(30px, 5vw, 60px)", lineHeight: 1, marginBottom: 14 }}>{movie.title}</h1>
            <div style={{ display: "flex", gap: 20, color: "#999", fontSize: 14, flexWrap: "wrap", marginBottom: 12 }}>
            ...
            {movie.cast && <p style={{ color: "#666", fontSize: 13 }}>Cast: {movie.cast}</p>}
            <p style={{ color: "#888", fontSize: 14, lineHeight: 1.75, maxWidth: 540, marginTop: 10 }}>{movie.description}</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto", padding: "40px 56px 80px" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <h2 style={{ fontSize: 22, fontWeight: 800, marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
            <ICONS.Calendar s={20}/> Select Date
          </h2>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {dates.map(d => (
              <button key={d.val} className={`btn btn-sm ${date === d.val ? "btn-red" : "btn-outline"}`} onClick={() => setDate(d.val)}>
                {d.label}
              </button>
            ))}
          </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {shows.map(show => (
                  <div key={show.id}
                    ...
                    style={{
                      background: selShow?.id === show.id ? "rgba(229,9,20,.07)" : "#0e0e0e",
                      border: `1.5px solid ${selShow?.id === show.id ? "#e50914" : "#1c1c1c"}`,
                      borderRadius: 13, padding: "18px 24px", cursor: "pointer",
                      transition: "all .22s", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 14,
                    }}
                    onMouseEnter={e => { if (selShow?.id !== show.id) e.currentTarget.style.borderColor = "#333"; }}
                    onMouseLeave={e => { if (selShow?.id !== show.id) e.currentTarget.style.borderColor = "#1c1c1c"; }}>
                    <div style={{ display: "flex", gap: 28, alignItems: "center" }}>
                      <div>
                        <div style={{ fontSize: 24, fontWeight: 900 }}>{show.startTime}</div>
                        <div style={{ fontSize: 11, color: "#555", marginTop: 2 }}>ends {show.endTime}</div>
                      </div>
                      <div>
                        <div style={{ fontSize: 16, fontWeight: 700, color: "#e0e0e0" }}>{show.theatreName}</div>
                        <div style={{ fontSize: 12, color: "#555", display: "flex", alignItems: "center", gap: 4, marginTop: 3 }}>
                          <ICONS.Pin s={11}/> {show.theatreCity} · {show.screenName} · <span style={{ color: "#666" }}>{show.screenType}</span>
                        </div>
                      </div>
                    </div>
                    <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontSize: 10, color: "#444", fontWeight: 700, letterSpacing: 1 }}>FROM</div>
                        <div style={{ fontSize: 22, fontWeight: 900, color: "#4ade80" }}>₹{minPrice(show)}</div>
                      </div>
                      <span style={{
                        padding: "6px 14px", borderRadius: 7, fontSize: 12, fontWeight: 700,
                        background: show.availableSeats > 20 ? "rgba(74,222,128,.1)" : show.availableSeats > 0 ? "rgba(229,9,20,.1)" : "#111",
                        color: show.availableSeats > 20 ? "#4ade80" : show.availableSeats > 0 ? "#e50914" : "#444",
                      }}>
          ...
          <div className="fade-up">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 8, marginBottom: 18 }}>
              <h2 style={{ fontSize: 22, fontWeight: 800, display: "flex", alignItems: "center", gap: 10 }}>
                <ICONS.Tv s={20}/> Choose Your Seats
              </h2>
              <span style={{ color: "#555", fontSize: 13 }}>{selShow.theatreName} · {selShow.startTime}</span>
            </div>

            <div style={{ background: "#0a0a0a", border: "1px solid #161616", borderRadius: 16, padding: "40px 28px" }}>
              ...
              <div className="slide-up" style={{
                ...
                <div>
                  ...
                  <div style={{ color: "#888", fontSize: 13 }}>
                    {[...new Set(selSeats.map(s => s.seatType))].join(", ")}
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ color: "#444", fontSize: 11, marginBottom: 3 }}>₹{totalAmt.toFixed(0)} + ₹{convFee.toFixed(0)} fee</div>
...
function BookingsPage() {
  ...
  useEffect(() => {
    api.get("/api/bookings/my-bookings", token)
      .then(setBookings).catch(e => toast(e.message, "error")).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div style={{ maxWidth: 920, margin: "0 auto", padding: "90px 52px 80px" }}>
      ...
      <div style={{ marginBottom: 40 }}>
        ...
        <p style={{ color: "#555", fontSize: 14 }}>{bookings.length} booking{bookings.length !== 1 ? "s" : ""} total</p>
      </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {bookings.map((b, i) => (
              <div key={b.id} className="fade-up" style={{
                ...
                padding: "24px 28px", animationDelay: `${i * .04}s`,
              }}>
                ...
                <div style={{ display: "flex", gap: 36, flexWrap: "wrap" }}>
                  {[
                    ...
                    <div key={label}>
                      <div style={{ color: "#444", fontSize: 10, fontWeight: 700, letterSpacing: 1.2, marginBottom: 5 }}>{label}</div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <Field label="Movie Title *">
        <input className="inp" placeholder="e.g. Kalki 2898-AD" value={form.title} onChange={set("title")}/>
      </Field>
      <Row>
      ...
      <Field label="Poster URL">
        <input className="inp" placeholder="https://image.tmdb.org/…" value={form.posterUrl} onChange={set("posterUrl")}/>
      </Field>
      <Field label="Trailer URL">
        <input className="inp" placeholder="https://youtube.com/watch?v=…" value={form.trailerUrl} onChange={set("trailerUrl")}/>
      </Field>
...
function AddShowForm({ onSuccess }) {
  ...
  useEffect(() => {
    ...
    api.get("/api/theatres").then(setTheatres).catch(() => {});
  }, []);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <p style={{ color: "#555", fontSize: 12, fontWeight: 700, letterSpacing: 1 }}>TICKET PRICES (leave blank if seat type not available)</p>
      <Row>
...
function AddTheatreForm({ onSuccess }) {
  ...
  const toast = useToast();
  const [loading, setLoading] = useState(false);
  const blank = { name:"", address:"", city:"", state:"", pinCode:"", phone:"" };
  const [form, setForm] = useState(blank);
const handleChange = (key) => (e) => {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      await api.post("/api/theatres", { ...form, active: true }, token);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <Field label="Theatre Name *"><input className="inp" placeholder="e.g. PVR Forum Mall" value={form.name} onChange={handleChange("name")}/></Field>
      <Field label="Full Address"><input className="inp" placeholder="Street, Area" value={form.address} onChange={handleChange("address")}/></Field>
      <Row>
        <Field label="City *"><input className="inp" placeholder="e.g. Bengaluru" value={form.city} onChange={handleChange("city")}/></Field>
        <Field label="State"><input className="inp" placeholder="e.g. Karnataka" value={form.state} onChange={handleChange("state")}/></Field>
      </Row>
      <Row>
        <Field label="PIN Code"><input className="inp" placeholder="560001" value={form.pinCode} onChange={handleChange("pinCode")}/></Field>
        <Field label="Phone"><input className="inp" placeholder="080-XXXXXXXX" value={form.phone} onChange={handleChange("phone")}/></Field>
      </Row>
      <button className="btn btn-red btn-md" onClick={submit} disabled={loading} style={{ width: "100%", justifyContent: "center" }}>
        <ICONS.Building s={18}/> {loading ? "Adding…" : "Add Theatre"}
      </button>
    </div>
  );
}
...
function AdminPage() {
  ...
  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      const [m, t] = await Promise.all([api.get("/api/movies"), api.get("/api/theatres")]);
  ...
  const TABS = [
    { id: "movies",   label: "Movies",   icon: <ICONS.Film s={16}/> },
    { id: "shows",    label: "Shows",    icon: <ICONS.Calendar s={16}/> },
    { id: "theatres", label: "Theatres", icon: <ICONS.Building s={16}/> },
  ];
  ...
  return (
    <div style={{ paddingTop: 78, minHeight: "100vh" }}>
      {/* Admin header */}
      <div style={{
        background: "linear-gradient(to right, rgba(229,9,20,.08), transparent)",
        borderBottom: "1px solid rgba(229,9,20,.12)", padding: "28px 52px 24px",
        display: "flex", alignItems: "center", gap: 20,
      }}>
        <div style={{ background: "#e50914", borderRadius: 14, padding: "12px 14px", display: "flex" }}>
          <ICONS.Shield s={26}/>
        </div>
        <div>
          <h1 className="display" style={{ fontSize: 50, letterSpacing: 3, marginBottom: 2 }}>ADMIN DASHBOARD</h1>
          <p style={{ color: "#555", fontSize: 14 }}>Manage your CineBook platform</p>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 0, maxWidth: 1200, margin: "0 auto", padding: "0 52px 80px" }}>
        ...
        <div style={{ width: 200, flexShrink: 0, paddingTop: 32 }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              ...
              background: tab === t.id ? "rgba(229,9,20,.12)" : "transparent",
              ...
              {t.icon} {t.label}
              <span style={{ marginLeft: "auto", background: tab === t.id ? "#e50914" : "#1e1e1e", color: tab === t.id ? "#fff" : "#555", borderRadius: 999, padding: "1px 8px", fontSize: 11, fontWeight: 700 }}>
                {t.id === "movies" ? movies.length : t.id === "theatres" ? theatres.length : ""}
              </span>
            </button>
          ))}
        </div>
        ...
        <div style={{ flex: 1, paddingLeft: 36, paddingTop: 32 }}>
          ...
          {tab === "movies" && (
            <div>
              {/* Add movie section */}
              <div style={{ background: "#0d0d0d", border: "1px solid #1c1c1c", borderRadius: 16, padding: 28, marginBottom: 32 }}>
              ...
              {loading ? <Spinner/> : movies.length === 0 ? <Empty icon="🎬" title="No movies yet"/> : (
                <div style={{ background: "#0d0d0d", border: "1px solid #1c1c1c", borderRadius: 16, overflow: "hidden" }}>
                  <table className="tbl">
                    <thead>
                      <tr>
                        <th>Movie</th><th>Genre</th><th>Language</th><th>Duration</th><th>Rating</th><th>Cert</th><th></th>
                      </tr>
                    </thead>
                    <tbody>
                      {movies.map(m => (
                        <tr key={m.id}>
                          <td>
                            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                              {m.posterUrl
                                ? <img src={m.posterUrl} alt="" style={{ width: 38, height: 56, objectFit: "cover", borderRadius: 5 }}/>
                                : <div style={{ width: 38, height: 56, background: "#1e1e1e", borderRadius: 5, display: "flex", alignItems: "center", justifyContent: "center" }}>🎬</div>
                              }
                              <div>
                                <div style={{ fontWeight: 700, color: "#fff" }}>{m.title}</div>
                                <div style={{ color: "#555", fontSize: 12 }}>{m.director}</div>
                              </div>
                            </div>
                          </td>
                          <td>{m.genre}</td>
                          ...
                          <td>
                            <button className="btn btn-danger btn-sm" onClick={() => deleteMovie(m.id, m.title)}>
                              <ICONS.Trash s={14}/>
                            </button>
                          </td>
                        </tr>
            ...
            <div>
              ...
              <div style={{ background: "#0d0d0d", border: "1px solid #1c1c1c", borderRadius: 16, padding: "20px 24px" }}>
                <p style={{ color: "#555", fontSize: 14 }}>
                  Shows are linked to a movie + theatre + screen. Once a show is scheduled, seats are auto-generated from the screen layout. View shows from the Movie Detail page.
                </p>
              </div>
            </div>
          )}
            ...
            <div>
              ...
              {loading ? <Spinner/> : theatres.length === 0 ? <Empty icon="🏢" title="No theatres yet"/> : (
                <div style={{ background: "#0d0d0d", border: "1px solid #1c1c1c", borderRadius: 16, overflow: "hidden" }}>
                  <table className="tbl">
                    <thead>
                      <tr><th>Name</th><th>City</th><th>Address</th><th>Screens</th><th>Status</th></tr>
                    </thead>
                    <tbody>
                      {theatres.map(t => (
                        <tr key={t.id}>
                          ...
                          <td>{t.city}</td>
                          <td style={{ color: "#555", fontSize: 13 }}>{t.address}</td>
                          <td>{t.screens?.length || 0} screen{(t.screens?.length || 0) !== 1 ? "s" : ""}</td>
